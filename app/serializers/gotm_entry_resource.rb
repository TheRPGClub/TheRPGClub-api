...
#
# The embed uses GameWithPlatformsResource — the shared embedded-game shape
# plus platforms, since the dashboard's GOTM card renders a pill per platform.
# GameResource is wrong here: it re-declares the `gotm_won`/`nr_gotm_won` SQL
# aliases, which only exist on records loaded through
# `GamedbGame.without_images`. GOTM games are loaded via
# `preload(game: [ :images, :platforms ])` (a plain all-columns select), so
